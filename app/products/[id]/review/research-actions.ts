...
import { researchShopeePublic } from "@/lib/marketplaces/shopee/public-research";
...
export async function retryShopeeResearch(productId: string) {
  ...
  const outcome = await saveResearchEvidence(supabase,productId,await researchShopeePublic(product));
