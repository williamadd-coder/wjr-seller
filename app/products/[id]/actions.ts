...
function buildTitle(product: Record<string, any>) { return [product.brand, product.name, product.model].filter(Boolean).join(" ").replace(/\s+/g, " ").trim().slice(0, 120); }
function buildDescription(product: Record<string, any>) { return [`Conheça ${product.name}, uma opção pensada para quem busca praticidade e uma compra bem informada.`,"","DESTAQUES DO PRODUTO",product.brand?`- Marca: ${product.brand}`:null,product.model?`- Modelo: ${product.model}`:null,product.ean?`- EAN: ${product.ean}`:null,product.weight_kg?`- Peso do pacote: ${product.weight_kg} kg`:null,product.width_cm&&product.height_cm&&product.length_cm?`- Dimensões do pacote: ${product.width_cm} x ${product.height_cm} x ${product.length_cm} cm`:null,"","Antes de publicar, confirme materiais, medidas do produto, conteúdo da embalagem, indicação de uso, idade recomendada e demais características específicas. Não incluímos informações que não estejam sustentadas pela fonte do produto."].filter((line)=>line!==null).join("\n"); }
export async function optimizeShopee(productId: string) {
 ...
 const title=buildTitle(product); const description=buildDescription(product);
 ...
 const keywords=product.name.toLowerCase().split(/\s+/).filter((word:string)=>word.length>3).slice(0,8); const imageCount=(assets??[]).filter((a)=>a.asset_type==="image").length; const hasVideo=(assets??[]).some((a)=>a.asset_type==="video"); const price=pricing?.sale_price==null?undefined:Number(pricing.sale_price);
 ...
 revalidatePath(`/products/${productId}`); revalidatePath(`/products/${productId}/package`); revalidatePath("/products");
}
