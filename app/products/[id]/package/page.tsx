...
export default async function FinalPackagePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const completePackage = pack.fields.map((item) => `[${item.section}] ${item.label} — ${item.status}:\n${display(item.value)}`).join("\n\n");
  ...
  return <main className="studioPage">
    ...
    <div className="studioHeader"><div><div className="eyebrow">Etapa 4 · Publicação manual</div><h1>Pacote Final para cadastrar na Shopee</h1><p>Informações organizadas para copiar e preencher o anúncio. Cada campo mostra se é confirmado, sugestão comercial ou depende de validação.</p></div><div className="scoreBadge"><span>Prontidão</span><strong>{pack.ready ? "100%" : "—"}</strong><small>{pack.ready ? "pronto para revisão" : `${pack.pending.length} pendência(s)`}</small></div></div>
