...
export default async function FinalPackagePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="studioPage">
    ...
    <section className="formSection packageSection"><div className="sectionTitle"><div><div className="eyebrow">Ordem de cadastro</div><h2>Dados do anúncio</h2></div><span className={`statusChip ${pack.ready ? "done" : ""}`}>{pack.ready ? "Pronto" : "Em preparação"}</span></div><div className="packageFields">{pack.fields.map(([label, value]) => <div className="packageField" key={String(label)}><span>{label}</span><div>{display(value)}</div></div>)}</div></section>
