...
export function scoreListing(draft: ListingDraft): ListingScore {
  ...
  if (draft.title.length > 120) { titleScore -= 45; blockers.push("Título excede o limite configurado"); }
  ...
  let mediaScore = Math.min(100, draft.imageCount * 14 + (draft.hasVideo ? 20 : 0));
